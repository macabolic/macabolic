class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  def facebook    
    if user_signed_in?
      logger.debug "I am now connecting to facebook within my macabolic session."
      logger.debug "Omniauth.auth: #{request.env["omniauth.auth"]}"
      #@user = User.find_for_facebook_oauth(request.env["omniauth.auth"], current_user)
      
      facebook_authentication = current_user.authentications.find_by_provider(Authentication::FACEBOOK)
      if !facebook_authentication.present?
        current_user.authentications.build(:provider => request.env["omniauth.auth"].provider, :uid => request.env["omniauth.auth"].uid, :token => request.env["omniauth.auth"].credentials.token)
        current_user.profile_images.build(:provider => request.env["omniauth.auth"].provider, :uid => request.env["omniauth.auth"].uid)
        current_user.save
      end 
      
      redirect_to session["user_return_to"]               
    else
      logger.debug "I am now logging in to macabolic via facebook."
  ...
  def twitter
    ...
    if user_signed_in?
      logger.debug "I am now connecting to twitter within my macabolic session."
      logger.debug "Omniauth.auth: #{request.env["omniauth.auth"]}"
      
      twitter_authentication = current_user.authentications.find_by_provider(Authentication::TWITTER)
